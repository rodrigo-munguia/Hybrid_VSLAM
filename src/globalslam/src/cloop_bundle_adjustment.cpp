...
void CLOOP::Update_gmap_with_BA(std::vector<int> &idx_vl_new_kf, std::vector<int> &idx_vl_old_kf)
{
    ...
    //------ define kf (and associated points) to be optimized
       ...
       // if( idx_kf_min_ref_for_search_matches < 0)idx_kf_min_ref_for_search_matches = 0;
        ...
        int n_old_kf = idx_vl_old_kf.size();
        int n_new_kf = idx_vl_new_kf.size();

        for (int i = 0; i < n_old_kf ; i++)
        { 
          int idx_kf_t = idx_vl_old_kf[i]; 
        ...
        int max_n_new_kf_2_opt = 7;
        for(int i = Gmap.KeyFDATA.size()-1; i >= Gmap.KeyFDATA.size()-max_n_new_kf_2_opt ;i-- )
        {
          if( find(idx_kf_to_optimize.begin(), idx_kf_to_optimize.end(), i) == idx_kf_to_optimize.end() )
          {  
           idx_kf_to_optimize.push_back(i);
          }       
        }
   ...
   cout << "cloop-> Local bundle adjustment for loop close refinement" << endl;
    ...
    {   
       ...
       cout << "cloop-> Kf: " << idx_kf << " n matches: " << Gmap.KeyFDATA[idx_kf].Idx_Matched_points.size()  << endl;
    ...
    for( int i = 0; i < idx_points.size(); i++)
    {
        ...
        gtsam::Point3 Pt;
         ...
         if (Gmap.AnchorsDATA[idx_pt].init_KF <= idx_vl_old_kf[idx_vl_old_kf.size()-1]) 
         {
              init_prior = true;
